...
fn extract_current_word(rope: &Rope, offset: usize) -> (String, usize) {
    ...
    (
        rope.slice(start..offset).to_string().to_uppercase(),
        start,
    )
}
...
fn suggest_items(
    ...
    let after_column = last_token
        .and_then(|t| schema.columns.iter().find(|c| c.name.eq_ignore_ascii_case(t)));

    // 检测上下文：是否在操作符后面（需要值提示）
    let after_operator = last_token.map(|t| {
        let upper = t.to_uppercase();
        matches!(upper.as_str(), "=" | "!=" | ">" | "<" | ">=" | "<=" | "LIKE" | "IN")
    }).unwrap_or(false);
...
fn suggest_value_templates(current_word: &str, range: Range, items: &mut Vec<CompletionItem>) {
    ...
    for (label, text, doc) in &templates {
        if label.to_uppercase().starts_with(&current_word.to_uppercase()) || current_word.is_empty()
        {
...
fn has_complete_condition(text: &str) -> bool {
    ...
    upper.contains('=') || upper.contains('>') || upper.contains('<') 
        || upper.contains("LIKE") || upper.contains("IN") || upper.contains("BETWEEN")
}
...
fn suggest_operators(
    ...
    let ops: Vec<(&str, &str, &str)> = if dt.contains("CHAR")
        || dt.contains("TEXT")
        || dt.contains("VARCHAR")
    {
        vec![
    ...
    for (label, text, doc) in ops {
        if !current_word.is_empty()
            && !label.to_uppercase().starts_with(&current_word.to_uppercase())
        {
            continue;
        }
...
impl CompletionProvider for OrderByCompletionProvider {
    ...
    fn is_completion_trigger(
        ...
        matches!(new_text, " " | "." | ",") ||
        new_text.chars().next().map_or(false, |c| c.is_alphabetic() || c == '_')
    }
}
    ...
    pub fn set_schema(&mut self, schema: TableSchema, cx: &mut Context<Self>) {
        ...
        self.where_editor.update(cx, |editor, cx| {
            editor.editor.update(cx, |input_state, _cx| {
                input_state.lsp.completion_provider = Some(Rc::new(WhereCompletionProvider::new(schema.clone())));
            });
        });

        self.order_by_editor.update(cx, |editor, cx| {
            editor.editor.update(cx, |input_state, _cx| {
                input_state.lsp.completion_provider = Some(Rc::new(OrderByCompletionProvider::new(schema_clone)));
            });
        });
    }
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        div()
                            .flex_1()
                            .child(self.order_by_editor.clone()),
                    ),
            )
    }
}