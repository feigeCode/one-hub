...
impl IconName {
    ...
    /// Return the icon in mono mode (uses text_color tinting)
    pub fn mono(self) -> Icon {
        Icon::build(self).mono()
    }

    /// Return the icon with specified color mode
    pub fn color_mode(self, mode: IconColorMode) -> Icon {
        Icon::build(self).color_mode(mode)
    }
}
